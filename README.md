...
这是宣博乐的独立个人学术网站仓库，基于 [al-folio](https://github.com/alshedivat/al-folio) 构建。

## 网站定位

本仓库用于长期维护个人学术主页，主要服务于博士申请、研究方向展示、论文与在研项目展示、学术简历，以及开放研究与研究工具展示。

- 正式维护仓库：`mohui373/Xuan`
- 网站地址：`https://mohui373.github.io/Xuan/`
- 唯一长期维护分支：`main`

今后所有人工内容修改都直接提交到 `main`。`gh-pages` 仅由 GitHub Actions 自动生成用于网站发布，不作为人工编辑分支。
...
3. **AI、组织变革与员工行为**  
   当前进行中的研究方向，比较 AI 驱动裁员与常规裁员对幸存员工心理和行为的不同影响，重点关注知识隐藏与工作态度等结果。
...
构建时读取 `_config.yml` 与 `_config_xuan.yml`。日常内容更新不要求在本地安装 Ruby、Jekyll 或 Docker。
...
不得为了完善页面而虚构投稿期刊、DOI、预印本、样本量、实验结果、引用次数、未确认奖项或未公开研究设计。

## 隐私原则

公开网页不展示手机号、出生日期、籍贯、家庭住址和未脱敏的完整个人材料。后续新增 Google Scholar、ORCID、OSF、CV PDF 等链接时，只使用已经确认可以公开的信息。

## 维护原则

这是个人长期学术主页，不是 al-folio 模板展示仓库，也不是科技产品官网。后续修改优先保证学术事实准确、研究主线清晰、页面简洁易读、移动端可用，以及内容结构可持续维护。
